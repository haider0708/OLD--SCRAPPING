#!/usr/bin/env python3
"""
bambinos.tn scraper — WooCommerce (Mikado theme), no bot protection (httpx).
Categories: /categorie-produit/{slug}/
Pagination: ?paged=N
Price format: "99,000 DT" (comma = thousands sep when 3 digits)
"""
import asyncio
import logging
import re
...
BASE = "https://bambinos.tn"


class BaminosScraper(FastScraper):
    """HTTP scraper for bambinos.tn (WooCommerce)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("bambinos", logger)
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("li.product.type-product, li.product, .product-grid-item"):
            product = {}
            ...
            link = item.css_first("a.woocommerce-LoopProduct-link, a[href*='/product/']")
            if not link:
                link = item.css_first("h6 a, h2 a, h3 a")
            if not link:
                continue
            ...
            name_el = item.css_first("h6.mkdf-product-list-title, h2.woocommerce-loop-product__title, .woocommerce-loop-product__title")
            if name_el:
    ...
    def extract_product_details(self, html: str, product: dict) -> dict:
        ...
        # WooCommerce: span.sku_wrapper span.sku
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting bambinos scrape")
...
def get_scraper(logger: logging.Logger) -> BaminosScraper:
    return BaminosScraper(logger)
