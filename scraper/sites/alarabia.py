#!/usr/bin/env python3
"""
alarabia.com.tn scraper — PrestaShop, no bot protection (httpx).
Categories: /{id}-{slug}
Pagination: ?page=N
Price format: "1,299,000 TND" or "999,000 TND" (comma = thousands sep)
"""
import asyncio
import logging
import re
...
BASE = "https://www.alarabia.com.tn"


class AlarabiaScraper(FastScraper):
    """HTTP scraper for alarabia.com.tn (PrestaShop)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("alarabia", logger)
    ...
    def _parse_price(self, text: Optional[str]) -> Optional[float]:
        ...
        if not cleaned:
            return None
        # "1,299,000" → multiple commas, last group 3 digits = all thousands seps
        parts = cleaned.split(",")
        if len(parts) > 2:
            # All commas are thousands seps: "1,299,000" → 1299.000
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='alarabia.com.tn/']"):
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css(".js-product-miniature, article.product-miniature"):
            product = {}
            ...
            link = item.css_first("a.product-link, a.thumbnail, a.product-thumbnail")
            if not link:
                link = item.css_first("h2 a, h3 a, .product-name a")
            if not link:
                continue
            ...
            name_el = item.css_first(".product-name, .product-title, h2, h3")
            if name_el:
            ...
            price_el = item.css_first(".price:not(.regular-price), .product-price .price")
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting alarabia scrape")
...
def get_scraper(logger: logging.Logger) -> AlarabiaScraper:
    return AlarabiaScraper(logger)
