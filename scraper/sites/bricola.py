#!/usr/bin/env python3
"""
bricola.tn scraper — PrestaShop 1.7, no bot protection (httpx).
Categories: /{id}-{slug}
Pagination: ?page=N
Price format: "18,90 DT" (comma = decimal) / "1 099,00 DT" (space = thousands sep)
SKU: "Référence XXXXXXX" span on detail page; EAN also in product URL
"""
import asyncio
import logging
import re
...
BASE = "https://bricola.tn"


class BricolaScraper(FastScraper):
    """HTTP scraper for bricola.tn (PrestaShop 1.7)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("bricola", logger)
    ...
    def _parse_price(self, text: Optional[str]) -> Optional[float]:
        if not text:
            return None
        # "1 099,00 DT" → space=thousands, comma=decimal
    ...
    def _ean_from_url(self, url: str) -> Optional[str]:
        """Extract EAN-13 from product URL slug."""
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='bricola.tn/']"):
            ...
            # PrestaShop category: {id}-{slug}  (no .html, no /)
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("div.product-miniature.js-product-miniature, .product-miniature"):
            ...
            link = item.css_first("a.thumbnail.product-thumbnail, a[href*='.html']")
            if not link:
                continue
            ...
            name_el = item.css_first("h2.product-title, h3.product-title, .product-title")
            if name_el:
            ...
            price_el = item.css_first("span.price, .price")
    ...
    def extract_product_details(self, html: str, product: dict) -> dict:
        ...
        # PrestaShop: "Référence XXXXXXX" in a span (no colon, no class)
        # Also try standard itemprop and EAN from URL
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting bricola scrape")
...
def get_scraper(logger: logging.Logger) -> BricolaScraper:
    return BricolaScraper(logger)
