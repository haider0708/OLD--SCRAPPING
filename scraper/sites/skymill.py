...
class SkymillScraper(FastScraper):
    ...
    async def download_frontpage(self):
        """Download frontpage. Use a FRESH browser context (not the shared one)
        to bypass Cloudflare — it tracks the shared context."""
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                # Give CF JS challenge time to resolve
            ...
            if resp and resp.status == 200 and not is_cf and len(html) > 50000:
                ...
                return output_path
            self.logger.warning(f"Attempt {attempt}: status={resp.status if resp else None}, len={len(html)}, cf={is_cf} — retrying")
            await asyncio.sleep(2 + attempt * 2)

        # Final save even if blocked, so categories.json doesn't crash
        save_text_atomic(html, output_path, self.logger)
        self.logger.warning(f"⚠️ Falling back to last-attempt content ({len(html):,} bytes)")
        return output_path
    ...
    async def scrape_product_details(self, url: str) -> dict:
        ...
        # Extract SKU from specs table (look for "Référence", "Réf", "SKU", "Code" key)
        data["sku"] = None
        for k, v in specs.items():
            if re.search(r"^(Référence|Réf|Ref|SKU|UGS|Code\s*produit|EAN|Barcode)", k, re.IGNORECASE):
                data["sku"] = v
                break
        # Fallback: scan page text
        if not data["sku"]:
            for el in tree.css("p, span, li, td, dt"):
                txt = el.text(strip=True)
                if re.match(r"^(Référence|Réf|Ref|SKU|UGS)\s*[:\-]\s*\S", txt, re.IGNORECASE):
