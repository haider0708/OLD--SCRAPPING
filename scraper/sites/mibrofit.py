#!/usr/bin/env python3
"""
Mibrofit scraper - WordPress/WooCommerce + Woodmart, HTTP/selectolax.
"""
...
class MibrofitScraper(FastScraper):
    """HTTP scraper for mibrofit.tn WooCommerce pages and Store API."""

    CATEGORY_PATH_PREFIX = "/categorie-produit/"
    PRODUCT_PATH_PREFIX = "/produit/"
    SITE_HOSTS = {"mibrofit.tn", "www.mibrofit.tn"}
    BRAND_FALLBACK = "Mibro"
    ...
    def __init__(self, logger: logging.Logger):
        super().__init__("mibrofit", logger)
    ...
    def _as_selectors(value: Any) -> List[str]:
        if isinstance(value, list):
            out: List[str] = []
            for item in value:
                out.extend(MibrofitScraper._as_selectors(item))
            return out
        if not value:
            return []
    ...
    def _first_srcset_url(value: Any, prefer_largest: bool = True) -> Optional[str]:
        ...
        return max(parsed, key=lambda item: item[0])[1] if parsed else entries[-1].split(" ", 1)[0]
    ...
    def _meaningful_reference(value: Any) -> Optional[str]:
        text = clean_text(value)
        if not text or text.upper() in {"N/A", "NA", "ND", "SKU"}:
            return None
        if len(text) > 80:
            return None
        return text.strip(" :;|-")
    ...
    def _canonical_site_url(self, parts: Any, path: str, query: str = "") -> str:
        ...
        return urlunsplit((scheme, "mibrofit.tn", path, query, ""))
    ...
    def _category_url(self, href: Any) -> Optional[str]:
        ...
        if slug in {"uncategorized", "non-classe"}:
            return None
        return self._canonical_site_url(parts, f"/categorie-produit/{slug}/")
    ...
    def _availability(
        ...
        if add_button:
            ...
            if button_text and re.search(r"ajouter|add to cart|commander|acheter|choix des options", button_text, re.I):
                return stock_text or "En stock", True
    ...
    async def download_frontpage(self):
        ...
        self.logger.info(f"Downloading Mibrofit frontpage: {self.base_url}")
        ...
        for filename, url in extra_fetches.items():
            ...
            text = meta.get("html")
            if not text:
                self.logger.debug(f"Failed Mibrofit auxiliary fetch: {url} ({meta.get('error')})")
                continue
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        discovered_urls = {url for url, _name in discovered}
        ordered_slugs: List[str] = []
        for url, _name in discovered + sitemap_links:
            slug = self._category_slug(url)
            if slug and slug in category_map and slug not in ordered_slugs:
        ...
        for slug in ordered_slugs:
            item = category_map[slug]
            url = self._category_url(item.get("permalink") or item.get("link")) or (
                f"https://mibrofit.tn/categorie-produit/{slug}/"
            )
            ...
            categories.append(
                {
                    ...
                    "validated_on_frontpage": url in discovered_urls,
                }
            )
    ...
    def _load_category_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        if cache_path.exists():
            try:
                ...
                self.logger.debug(f"Failed reading Mibrofit Store API category cache: {exc}")
        ...
        if payload is None:
            ...
            if api_url:
                try:
                    ...
                    self.logger.debug(f"Mibrofit Store API category fallback failed: {exc}")
                    payload = []
    ...
    def _category_links_from_html(self, tree: HTMLParser) -> List[Tuple[str, str]]:
        selector = self.selectors.get("frontpage", {}).get(
            "category_links",
            "header a[href*='/categorie-produit/'], footer a[href*='/categorie-produit/']",
        )
    ...
    async def scrape_category_page(self, url: str) -> dict:
        ...
        if html and not (self.html_dir / "listing_sample_1.html").exists() and self._category_slug(url):
            save_text_atomic(html, self.html_dir / "listing_sample_1.html", self.logger)

        category_slug = self._category_slug(url)
        api_result = await self._scrape_store_api_category_page(url)
        if category_slug and not api_result.get("error"):
            if html:
                html_pagination = self.extract_pagination_from_html(html)
                api_result["pagination"]["html_pagination"] = html_pagination
            return api_result
        ...
        try:
            ...
            self.logger.debug(f"Error parsing Mibrofit category {url}: {exc}")
            return {
                "products": [],
                "pagination": {"current_page": self._page_num_from_url(url), "total_pages": 1, "has_next": False},
                "error": str(exc),
    ...
    def extract_products_from_html(self, html: str) -> List[dict]:
        ...
        cards = []
        seen_nodes = set()
        for selector in self._as_selectors(cp.get("item_selectors")):
        ...
        return dedupe_products(products, self.logger, "mibrofit listing")
    ...
    def _product_from_card(self, card: Any, selectors: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        price, old_price = self._price_pair(
            ...
            [selectors.get("item_price", "span.price"), ".price .woocommerce-Price-amount", ".price"],
        )
        ...
        image = self._image_from_node(self._first(card, selectors.get("item_image", [])))
    ...
    async def _scrape_store_api_category_page(self, category_url: str) -> dict:
        ...
        if not category:
            return {"products": [], "pagination": {"current_page": page, "total_pages": 1, "has_next": False}}

        per_page = int(self.config.get("settings", {}).get("api_per_page", 20))
        ...
        products = [
            product
            for product in (self._product_from_store_api(item) for item in payload)
            if product
        ]
        try:
        ...
        return {
            "products": dedupe_products(products, self.logger, "mibrofit api listing"),
    ...
    def _product_from_store_api(self, item: Any) -> Optional[Dict[str, Any]]:
        ...
        availability, available = self._availability(
            text=stock.get("text") or stock.get("class"),
        ...
        brand = None
        for brand_item in item.get("brands") or []:
            if isinstance(brand_item, dict):
                brand = clean_text(brand_item.get("name"))
                if brand:
                    break
        brand = brand or self.BRAND_FALLBACK
    ...
    def build_page_url(self, base_url: str, page_num: int) -> str:
        ...
        return urlunsplit((parts.scheme or "https", parts.netloc or "mibrofit.tn", path, urlencode(query), ""))
    ...
    def extract_pagination_from_html(self, html: str) -> dict:
        ...
        if current_page >= total_pages:
            has_next = False
        return {"current_page": current_page, "total_pages": total_pages, "has_next": has_next or current_page < total_pages}
    ...
    async def scrape_all_pages(self, category_url: str, limit: int = None) -> List[dict]:
        ...
        if limit and len(all_products) >= limit:
            return dedupe_products(all_products[:limit], self.logger, "mibrofit limited listing")
        total_pages = int(first.get("pagination", {}).get("total_pages") or 1)
        total_pages = min(total_pages, int(self.config.get("settings", {}).get("max_pagination_pages", 80)))
        for page in range(2, total_pages + 1):
            ...
            if limit and len(all_products) >= limit:
                break
        deduped = dedupe_products(all_products, self.logger, "mibrofit listing")
    ...
    async def scrape_product_details(self, url: str) -> dict:
        ...
        if api_product:
            ...
            await self._apply_variation_details(api_product, data)
        ...
        data.setdefault("brand", self.BRAND_FALLBACK)
    ...
    def _detail_product_id(self, tree: HTMLParser, data: Dict[str, Any]) -> Optional[str]:
        ...
        if not product_id:
            id_node = self._first(
                tree,
                [
                    ".summary button[name='add-to-cart'][value]",
                    ".entry-summary button[name='add-to-cart'][value]",
                    "form.cart button[name='add-to-cart'][value]",
                    "form.cart input[name='product_id'][value]",
                    "form.variations_form input[name='product_id'][value]",
                    ".single_add_to_cart_button[value]",
                ],
            )
            product_id = (
    ...
    async def _apply_variation_details(self, api_product: Dict[str, Any], data: Dict[str, Any]) -> None:
        variants = []
        for variation in api_product.get("variations") or []:
            variation_id = variation.get("id") if isinstance(variation, dict) else variation
            variation_id = clean_text(variation_id)
            if not variation_id:
                continue
            var_data = await self._fetch_product_api(variation_id)
            if not var_data:
                continue
            price, old_price = self._api_price_values(var_data.get("prices"))
            stock = var_data.get("stock_availability") if isinstance(var_data.get("stock_availability"), dict) else {}
            availability, available = self._availability(
                text=stock.get("text") or stock.get("class"),
                classes=stock.get("class"),
                api_available=var_data.get("is_in_stock"),
            )
            sku = self._meaningful_reference(var_data.get("sku"))
            barcode = normalize_gtin(sku) if sku else None
            images = []
            for image in var_data.get("images") or []:
                if isinstance(image, dict):
                    image_url = self._absolute_url(image.get("src") or image.get("thumbnail"))
                    if image_url:
                        images.append(image_url)
            variant = {
                ...
                "barcode": barcode,
                "variation": clean_text(var_data.get("variation")),
                "price": price,
                "old_price": old_price,
                "discount_percent": self._computed_discount(price, old_price),
                "availability": availability,
                "available": available,
                "image": images[0] if images else None,
                "images": self._dedupe_values(images) or None,
            }
            if isinstance(variation, dict) and variation.get("attributes"):
                variant["attributes"] = {
                    clean_text(attr.get("name")): clean_text(attr.get("value"))
                    for attr in variation.get("attributes")
                    if isinstance(attr, dict) and clean_text(attr.get("name")) and clean_text(attr.get("value"))
                }
        ...
        if variants:
            ...
            for variant in variants:
                image_urls.extend(variant.get("images") or ([variant["image"]] if variant.get("image") else []))
            ...
            if image_urls:
                data["images"] = self._dedupe_values(image_urls)
                data["image"] = data["images"][0]

    def _apply_detail_identifiers(self, tree: HTMLParser, data: Dict[str, Any]) -> None:
        sku_node = self._first(tree, self.selectors.get("product_page", {}).get("sku", []))
    ...
    def _apply_detail_title_price(self, tree: HTMLParser, data: Dict[str, Any]) -> None:
        ...
        if price is not None and not data.get("price"):
            data["price"] = price
    ...
    def _apply_detail_descriptions(self, tree: HTMLParser, data: Dict[str, Any]) -> None:
        ...
        if descriptions and len(descriptions[0] or "") > len(data.get("description") or ""):
    ...
    def _apply_detail_images(self, tree: HTMLParser, data: Dict[str, Any]) -> None:
        ...
        for variant in data.get("variants") or []:
            if isinstance(variant, dict):
                images.extend(variant.get("images") or ([variant["image"]] if variant.get("image") else []))
        images = self._dedupe_values(images)
        if images:
            data["images"] = images
            data["image"] = images[0]
...
def get_scraper(logger: logging.Logger) -> MibrofitScraper:
    """Factory used by scraper.sites registry."""
    return MibrofitScraper(logger)
