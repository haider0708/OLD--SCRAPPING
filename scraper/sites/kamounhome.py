#!/usr/bin/env python3
"""
KamounHome (kamounhome.tn) scraper.
Fast: WooCommerce with Woodmart theme, URLs like /boutique/<top>/<low>/.
"""
import logging
import re
...
class KamounhomeScraper(FastScraper):
    """Fast scraper for kamounhome.tn (WooCommerce / Woodmart)."""
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        # Woodmart: li.item-level-1 is top, li.item-level-2 is low
        top_blocks = tree.css("li.menu-item.item-level-1.menu-item-has-children")
        if not top_blocks:
            top_blocks = tree.css("li.menu-item-has-children")

        self.logger.info(f"Found {len(top_blocks)} top-level blocks")

        for top_li in top_blocks:
            top_link = top_li.css_first("a.woodmart-nav-link, a")
            if not top_link:
                continue
            ...
            if not top_name or not top_url or top_url in seen:
                continue
            # Filter to product category URLs (Woodmart uses /boutique/)
            if "/boutique/" not in top_url and "/categorie-produit/" not in top_url and "/product-category/" not in top_url:
                continue
            seen.add(top_url)

            top_cat = {"name": top_name, "url": top_url, "level": "top", "low_level_categories": []}

            # Low-level: li.item-level-2 inside this top
            for low_li in top_li.css("li.item-level-2, ul.sub-menu > li"):
                low_link = low_li.css_first("a.woodmart-nav-link, a")
                if not low_link:
                    continue
                ...
                for sub_li in low_li.css("li.item-level-3, ul.sub-menu > li"):
                    sub_link = sub_li.css_first("a.woodmart-nav-link, a")
                    if not sub_link:
                        continue
    ...
    def extract_products_from_html(self, html: str) -> List[dict]:
        ...
        for item in tree.css("div.product.type-product, li.product.type-product, ul.products li.product"):
            link = item.css_first("a.product-image-link, a.woocommerce-LoopProduct-link, a.woocommerce-loop-product__link")
            if not link:
                link = item.css_first("h3 a, h2.woocommerce-loop-product__title a, h3.wd-entities-title a")
            url = self._make_absolute_url(link.attributes.get("href", "")) if link else None
            if not url or url in seen_urls:
                continue
            seen_urls.add(url)

            name_el = item.css_first("h3.wd-entities-title, h2.woocommerce-loop-product__title, h3.product-title")
    ...
    def extract_pagination_from_html(self, html: str) -> dict:
        tree = HTMLParser(html)
        max_page = 1
        for a in tree.css("ul.page-numbers li a.page-numbers:not(.next):not(.prev), nav.woocommerce-pagination a.page-numbers"):
            try:
