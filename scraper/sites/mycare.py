#!/usr/bin/env python3
"""
mycare.tn scraper — PrestaShop (httpx).
Categories: /{id}-{slug}        (no .html suffix)
Products:   /{category}/{id}-{slug}-{ean}.html
Pagination: ?page=N
"""
import asyncio
import logging
...
BASE = "https://www.mycare.tn"


class MycareScraper(FastScraper):
    """HTTP scraper for mycare.tn (PrestaShop)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("mycare", logger)
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='mycare.tn/']"):
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting mycare scrape")
...
def get_scraper(logger: logging.Logger) -> MycareScraper:
    return MycareScraper(logger)
