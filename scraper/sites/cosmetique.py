#!/usr/bin/env python3
"""
cosmetique.tn scraper — PrestaShop, no bot protection (httpx).
Categories: /{id}-{slug}
Pagination: ?page=N
Price format: "10,000 TND" (comma = thousands sep when 3 digits)
"""
import asyncio
import logging
import re
...
BASE = "https://cosmetique.tn"


class CosmetiqueScraper(FastScraper):
    """HTTP scraper for cosmetique.tn (PrestaShop)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("cosmetique", logger)
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='cosmetique.tn/']"):
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("article.product-miniature, .product-miniature-default, .js-product-miniature"):
            product = {}
            ...
            # Name from title attribute or text
            ...
            img = item.css_first("img.img-fluid.js-lazy-product-image, img[data-src], img")
            if img:
    ...
    def extract_product_details(self, html: str, product: dict) -> dict:
        ...
        price_el = tree.css_first(".current-price span.price, [itemprop='price'], span.product-price")
        if price_el:
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting cosmetique scrape")
...
def get_scraper(logger: logging.Logger) -> CosmetiqueScraper:
    return CosmetiqueScraper(logger)
