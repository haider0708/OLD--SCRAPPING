#!/usr/bin/env python3
"""
mbm-tn.com scraper — PrestaShop, Cloudflare (needs Playwright).
Categories: /{id}-{slug}
Products:   /{category}/{id}-{slug}.html
Pagination: ?page=N
Price format: "49,000 TND" (comma = thousands separator, not decimal)
"""
import asyncio
import logging
import re
...
BASE = "https://mbm-tn.com"


class MbmScraper(FastScraper):
    """Playwright scraper for mbm-tn.com (PrestaShop + Cloudflare)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("mbm", logger)
        self._pw = None
        self._browser = None
        self._ctx = None
    ...
    def _parse_price(self, text: str) -> Optional[float]:
        if not text:
            return None
        # Format: "49,000 TND" — comma is thousands separator
        cleaned = re.sub(r"[^\d,.]", "", text).strip()
        if not cleaned:
            return None
        # "49,000" → remove comma (thousands sep) → 49000 → /1000 not needed, it's 49.000 TND
        # Actually PrestaShop TN: "49,000" means 49.000 TND (49 dinars)
        # Pattern: digits,3digits = thousands → "49,000" = 49000 millimes = 49 TND
        # But "399,900" = 399.900 TND
        ...
        if m2:
            # "49,90" → 49.90 TND (decimal comma)
            return float(f"{m2.group(1)}.{m2.group(2)}")
        # Remove all commas, try as integer millimes or plain float
        cleaned = cleaned.replace(",", "")
        try:
            val = float(cleaned)
            # If value looks like millimes (>1000 and no decimal), divide by 1000
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("article.product-miniature, .js-product-miniature"):
            ...
            name_el = item.css_first(".tvproduct-name-price-wrapper a, h2.h3 a, .product-title a, h2 a")
            if name_el:
            ...
            if img:
                ...
                if src and not src.startswith("data:"):
                    # Upgrade home_default to large_default
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting mbm scrape")
        await self._ensure_browser()
...
def get_scraper(logger: logging.Logger) -> MbmScraper:
    return MbmScraper(logger)
