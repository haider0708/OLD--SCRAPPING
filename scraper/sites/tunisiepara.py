#!/usr/bin/env python3
"""
tunisiepara.com scraper — WooCommerce + Flatsome, Cloudflare (needs Playwright).
Categories: /categorie/{slug}/
Pagination: /page/{n}/
Price format: "29,900 DT" (comma = thousands sep when 3 digits)
"""
import asyncio
import logging
import re
...
BASE = "https://tunisiepara.com"


class TunisieParaScraper(FastScraper):
    """Playwright scraper for tunisiepara.com (WooCommerce + Cloudflare)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("tunisiepara", logger)
        self._pw = None
        self._browser = None
        self._ctx = None
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("li.product, .product-grid-item, .type-product"):
            product = {}

            pid = item.attributes.get("data-id") or item.attributes.get("id", "")
            if pid:
                m = re.search(r"\d+", str(pid))
                if m:
                    product["id"] = m.group(0)
            ...
            name_el = item.css_first("h2.woocommerce-loop-product__title, h3.woocommerce-loop-product__title")
            if name_el:
    ...
    def has_next_page(self, html: str, current_page: int) -> bool:
        ...
        return tree.css_first(
            f"a.next.page-numbers, a[href*='/page/{current_page + 1}/']"
        ) is not None
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting tunisiepara scrape")
        await self._ensure_browser()
...
def get_scraper(logger: logging.Logger) -> TunisieParaScraper:
    return TunisieParaScraper(logger)
