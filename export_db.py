...
CLUSTER3_SHOPS = {"dokani", "ikitchen", "imag", "electrochaabani", "koktahome", "mageekstore", "electrohadjkacem", "capricelingerie", "tuttosport", "kastelo", "mbm", "infotec", "eleganza", "try_and_buy", "kiabi", "lesportif", "supersport", "tunisiepara", "paraexpert", "totaltunisia", "pointm", "cosmetique", "alarabia", "informatica", "paraland", "sangour", "farmasi", "sweetbaby", "bambinos", "bb_store", "toopty", "oriflame", "petit_bateau", "benyaghlane", "ceresbookshop", "alkitab", "culturel", "bricola", "piscineshop", "sqes", "petstore", "animalia", "zanimo", "celio", "animalzone", "zanimax", "kitty-city", "chicopets", "coquette", "maparatunisie", "lagrandepara", "mycare"}
...
class MongoDBExporter:
    def __init__(self):
        ...
        # Connect Atlas (primary) — handles shops NOT in NEW_DB_SHOPS and NOT in CLUSTER3_SHOPS
        if self.atlas_uri:
            try:
                client = MongoClient(
                    self.atlas_uri,
                ...
                self.clients.append((
                    ...
                    client,
                    self.db_name,
                    lambda s: s not in NEW_DB_SHOPS and s not in CLUSTER3_SHOPS,
                ))
        ...
        # Connect Atlas (cluster3) — handles shops in CLUSTER3_SHOPS
        if self.atlas_uri_3:
            try:
                client = MongoClient(
                    self.atlas_uri_3,
                ...
                self.clients.append((
                    ...
                    client,
                    self.db_name_3,
                    lambda s: s in CLUSTER3_SHOPS,
                ))
                logger.info(f"✅ Connected to Atlas MongoDB (cluster3 — for {sorted(CLUSTER3_SHOPS)})")
            except Exception as e:
                logger.warning(f"⚠️ Could not connect to Atlas MongoDB (cluster3): {e}")
...
def export_latest_run(site: str = None):
    """Finds the latest run directories for each shop and exports them.

    If site is given, exports only that single shop.
    """
    ...
    # If a specific site is requested, restrict to just that shop
    if site:
        shops = [site] if site in shops else []
...
if __name__ == "__main__":
    ...
    _parser.add_argument("--site", default=None, help="Export only this site")
    _parser.add_argument("path", nargs="?", default=None, help="Export a specific file or directory path")
    ...
    if _args.path:
        ...
        export_latest_run(site=_args.site)
