...
async def run_full_scrape(site_name, num_workers=16, detail_workers=64, limit=None, logger=None, scrape_details=True, use_detail_cache=True):
    ...
    t0 = time.time()
    try:
        if hasattr(scraper, "scrape_categories_async"):
            cat_data = await scraper.scrape_categories_async()
            cat_path = scraper.data_dir / "categories.json"
        else:
    ...
    if scrape_details and total_prods > 0:
        ...
        # If the scraper has an internal page semaphore (Playwright-hybrid), cap detail
        # workers to match — otherwise 64 tasks pile up waiting and time out before they
        # even get a browser page.
        effective_detail_workers = detail_workers
        if is_fast_scraper(scraper) and hasattr(scraper, '_page_sem'):
            sem_value = getattr(scraper._page_sem, '_value', None)
            if sem_value is not None:
                effective_detail_workers = min(detail_workers, sem_value)
        ...
        try:
            if items:
                if is_fast_scraper(scraper):
                    det_res = await scrape_details_fast(scraper, items, effective_detail_workers, pbar)
                else:
                    det_res = await scrape_details_playwright(scraper, items, effective_detail_workers, pbar)
            else:
                det_res = {}
        finally:
            if pbar is not None:
...
async def main():
    ...
    if args.cmd == "test":
        ...
        if args.export:
            ...
            export_latest_run(site=args.site if hasattr(args, "site") and args.site else None)
        ...
        if args.export:
            from export_db import export_latest_run
            export_latest_run(site=args.site)
