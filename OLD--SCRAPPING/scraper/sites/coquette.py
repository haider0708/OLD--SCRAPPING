#!/usr/bin/env python3
"""
coquette.tn scraper — PrestaShop (httpx).
Categories: /{id}-{slug}
Pagination: ?page=N
"""
import asyncio
...
BASE = "https://www.coquette.tn"


class CoquetteScraper(FastScraper):
    """HTTP scraper for coquette.tn (PrestaShop)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("coquette", logger)
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='coquette.tn/']"):
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting coquette scrape")
...
def get_scraper(logger: logging.Logger) -> CoquetteScraper:
    return CoquetteScraper(logger)
