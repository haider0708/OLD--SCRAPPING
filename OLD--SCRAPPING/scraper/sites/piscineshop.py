#!/usr/bin/env python3
"""
piscineshop.tn scraper — WooCommerce, no bot protection (httpx).
Category URL: /product-category/{slug}/
Pagination: /page/N/
Price: div.product-price ins (sale) or span.woocommerce-Price-amount
SKU: data-product_id or from URL slug
"""
import asyncio
import logging
import re
...
BASE = "https://piscineshop.tn"


class PiscineshopScraper(FastScraper):

    def __init__(self, logger: logging.Logger):
        super().__init__("piscineshop", logger)
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("li.product, li.type-product"):
            ...
            # Sale price in <ins>, regular in <del>
...
def get_scraper(logger: logging.Logger) -> PiscineshopScraper:
    return PiscineshopScraper(logger)
