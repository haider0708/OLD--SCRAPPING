#!/usr/bin/env python3
"""
paraexpert.tn scraper — WooCommerce + Moderno, no bot protection (httpx).
Categories: /product-category/{slug}/
Pagination: ?paged=N
Price format: "27.900 DT" (dot = thousands sep → 27.900 TND)
"""
import asyncio
import logging
import re
...
BASE = "https://www.paraexpert.tn"


class ParaexpertScraper(FastScraper):
    """HTTP scraper for paraexpert.tn (WooCommerce)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("paraexpert", logger)
    ...
    def _parse_price(self, text: Optional[str]) -> Optional[float]:
        if not text:
            return None
        # "27.900 DT" — dot = thousands sep → 27.900 TND
        # "74.900 DT" → 74.900 TND
        cleaned = re.sub(r"[^\d,.]", "", str(text)).strip()
        if not cleaned:
            return None
        # dot with 3 digits after = thousands sep
        m = re.match(r"^(\d+)\.(\d{3})$", cleaned)
        if m:
            return float(f"{m.group(1)}.{m.group(2)}")
        # comma with 3 digits = thousands sep
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("div.c-product-grid__item, li.product, .type-product"):
            product = {}

            pid = item.attributes.get("data-id")
            if not pid:
                # try from class like "post-1234"
            ...
            link = item.css_first("a.woocommerce-LoopProduct-link")
            if not link:
            ...
            name_el = item.css_first("h2.woocommerce-loop-product__title span.c-product-grid__title, h2.woocommerce-loop-product__title, h3")
            if name_el:
                product["name"] = self._clean(name_el.text())

            img = item.css_first("img.c-product-grid__thumb, img")
            if img:
    ...
    def has_next_page(self, html: str, current_page: int) -> bool:
        ...
        return tree.css_first(
            f"a.next.page-numbers, a[href*='paged={current_page + 1}'], "
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting paraexpert scrape")
...
def get_scraper(logger: logging.Logger) -> ParaexpertScraper:
    return ParaexpertScraper(logger)
