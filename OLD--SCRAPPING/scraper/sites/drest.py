#!/usr/bin/env python3
"""
Drest.tn scraper — WordPress + WooCommerce + Electro theme. httpx OK.
Categories: /product-category/{slug}/, pagination: /page/N/, products: /product/{slug}.
"""

import logging
import re
from typing import List, Optional
from selectolax.parser import HTMLParser
from scraper.base import FastScraper


class DrestScraper(FastScraper):

    def __init__(self, logger: logging.Logger):
        super().__init__("drest", logger)

    def build_page_url(self, base_url: str, page_num: int) -> str:
        base = re.sub(r"/page/\d+/?$", "", base_url.rstrip("/"))
        return f"{base}/page/{page_num}/"

    @staticmethod
    def _first_child_link(node):
        for child in node.iter():
            if child.tag == "a":
                return child
        return None

    def _absolute_url(self, href: str) -> str:
        if not href:
            return href
        if href.startswith("http"):
            return href
        if href.startswith("//"):
            return "https:" + href
        if href.startswith("/"):
            return f"https://www.drest.tn{href}"
        return href

    @staticmethod
    def _is_category_url(href: str) -> bool:
        return "/product-category/" in href and "/page/" not in href

    def extract_categories_from_html(self, html: str) -> dict:
        tree = HTMLParser(html)
        seen_urls = set()
        # Scan all /product-category/ hrefs and group by path depth
        # (drest has 592 unique category URLs across many levels)
        url_to_name: dict = {}
        for a in tree.css("a[href*='/product-category/']"):
            href = self._absolute_url(a.attributes.get("href", ""))
            if not self._is_category_url(href):
                continue
            href = href.rstrip("/") + "/"
            name = a.text(strip=True)
            if not name:
                continue
            if href not in url_to_name:
                url_to_name[href] = name

        # Build hierarchy by path segments after /product-category/
        tops: dict = {}
        lows: dict = {}
        subs: dict = {}

        for href, name in url_to_name.items():
            path = re.sub(r"^https?://[^/]+", "", href).strip("/")
            parts = path.split("/")
            # ['product-category', 'femme', 'accessoires-femme', ...]
            if len(parts) < 2 or parts[0] != "product-category":
                continue
            segs = parts[1:]
            if len(segs) == 1:
                slug = segs[0]
                tops.setdefault(slug, {
                    "name": name, "url": href.rstrip("/"),
                    "level": "top", "low_level_categories": [],
                })
                tops[slug]["name"] = name
            elif len(segs) == 2:
                key = f"{segs[0]}/{segs[1]}"
                lows.setdefault(key, {
                    "name": name, "url": href.rstrip("/"),
                    "level": "low", "subcategories": [],
                })
                if segs[0] not in tops:
                    tops[segs[0]] = {
                        "name": segs[0].replace("-", " ").title(),
                        "url": f"https://www.drest.tn/product-category/{segs[0]}",
                        "level": "top", "low_level_categories": [],
                    }
            else:
                key_low = f"{segs[0]}/{segs[1]}"
                key_sub = f"{segs[0]}/{segs[1]}/{segs[2]}"
                subs.setdefault(key_sub, {
                    "name": name, "url": href.rstrip("/"),
                    "level": "subcategory",
                })
                if key_low not in lows:
                    lows[key_low] = {
                        "name": segs[1].replace("-", " ").title(),
                        "url": f"https://www.drest.tn/product-category/{segs[0]}/{segs[1]}",
                        "level": "low", "subcategories": [],
                    }
                if segs[0] not in tops:
                    tops[segs[0]] = {
                        "name": segs[0].replace("-", " ").title(),
                        "url": f"https://www.drest.tn/product-category/{segs[0]}",
                        "level": "top", "low_level_categories": [],
                    }

        # Attach subs to lows
        for key_sub, sub_cat in subs.items():
            parts = key_sub.split("/")
            key_low = f"{parts[0]}/{parts[1]}"
            if key_low in lows:
                existing = {c["url"] for c in lows[key_low]["subcategories"]}
                if sub_cat["url"] not in existing:
                    lows[key_low]["subcategories"].append(sub_cat)

        # Attach lows to tops
        for key_low, low_cat in lows.items():
            parent_slug = key_low.split("/")[0]
            if parent_slug in tops:
                existing = {c["url"] for c in tops[parent_slug]["low_level_categories"]}
                if low_cat["url"] not in existing:
                    tops[parent_slug]["low_level_categories"].append(low_cat)

        categories = list(tops.values())
        stats = {"top_level": len(categories), "low_level": 0, "subcategory": 0, "total_urls": 0}
        for top in categories:
            if top.get("url"):
                stats["total_urls"] += 1
            for low in top["low_level_categories"]:
                stats["low_level"] += 1
                if low.get("url"):
                    stats["total_urls"] += 1
                for sub in low["subcategories"]:
                    stats["subcategory"] += 1
                    if sub.get("url"):
                        stats["total_urls"] += 1
        self.logger.info(f"Extracted {stats['top_level']} top, {stats['low_level']} low, {stats['subcategory']} sub")
        return {"categories": categories, "stats": stats}

    def extract_products_from_html(self, html: str) -> List[dict]:
        tree = HTMLParser(html)
        products = []
        seen_urls = set()

        for card in tree.css("li.product.type-product, div.product.type-product, section.product.type-product"):
            cls = card.attributes.get("class", "")

            # URL — prefer /product/ link
            link = card.css_first("a[href*='/product/']")
            if not link:
                link = card.css_first("h2 a, h3 a, a.woocommerce-LoopProduct-link, a[href]")
            if not link:
                continue
            href = link.attributes.get("href", "")
            url = self._absolute_url(href)
            if not url or url in seen_urls:
                continue
            if "/product-category/" in url or "?add-to-cart=" in url:
                continue
            seen_urls.add(url)

            # Name
            name_el = card.css_first(
                "h2.woocommerce-loop-product__title, h3.woocommerce-loop-product__title, "
                "h2.product-title, h3.product-title, .product-title"
            )
            name = name_el.text(strip=True) if name_el else link.text(strip=True)

            # Price
            ins_el = card.css_first("span.price ins span.woocommerce-Price-amount bdi, ins .woocommerce-Price-amount bdi")
            del_el = card.css_first("span.price del span.woocommerce-Price-amount bdi, del .woocommerce-Price-amount bdi")
            price_el = card.css_first("span.price span.woocommerce-Price-amount bdi, .price .woocommerce-Price-amount bdi")
            if ins_el and del_el:
                price = self._parse_price(ins_el.text())
                old_price = self._parse_price(del_el.text())
            else:
                price = self._parse_price(price_el.text() if price_el else None)
                old_price = None

            # SKU from add-to-cart button
            sku = None
            pid = None
            sku_btn = card.css_first("a.add_to_cart_button[data-product_sku], button[data-product_sku]")
            if sku_btn:
                sku = sku_btn.attributes.get("data-product_sku")
                pid = sku_btn.attributes.get("data-product_id")
            if not pid:
                m = re.search(r"\bpost-(\d+)\b", cls)
                if m:
                    pid = m.group(1)

            # Image
            img = card.css_first("img.wp-post-image, img.attachment-woocommerce_thumbnail, img")
            image = None
            if img:
                image = img.attributes.get("src") or img.attributes.get("data-src")
                if image and image.startswith("data:"):
                    image = None

            # Availability from class
            available = None
            if "instock" in cls:
                available = True
            elif "outofstock" in cls:
                available = False

            products.append({
                "id": pid, "url": url, "name": name, "sku": sku,
                "price": price, "old_price": old_price, "image": image,
                "available": available,
            })
        return products

    def extract_pagination_from_html(self, html: str) -> dict:
        tree = HTMLParser(html)
        max_page = 1
        for a in tree.css("nav.woocommerce-pagination a.page-numbers, ul.page-numbers a"):
            try:
                num = int(a.text(strip=True))
                if num > max_page:
                    max_page = num
            except ValueError:
                pass
        has_next = tree.css_first("a.next.page-numbers") is not None
        return {"current_page": 1, "total_pages": max_page, "has_next": has_next}

    def _parse_price(self, text: Optional[str]) -> Optional[float]:
        if not text:
            return None
        s = str(text).replace("\xa0", "").replace(" ", "")
        cleaned = re.sub(r"[^\d.,]", "", s).strip()
        if not cleaned:
            return None
        if "," in cleaned and "." in cleaned:
            cleaned = cleaned.replace(",", "")
        elif "," in cleaned:
            cleaned = cleaned.replace(",", "")
        try:
            return float(cleaned)
        except ValueError:
            return None

    async def scrape_product_details(self, url: str) -> dict:
        html = await self.fetch_html(url)
        if not html:
            return {"url": url, "error": "Failed to fetch"}

        tree = HTMLParser(html)
        data = {"url": url}

        body = tree.css_first("body")
        if body:
            m = re.search(r"\bpostid-(\d+)\b", body.attributes.get("class", ""))
            data["product_id"] = m.group(1) if m else None

        title_el = tree.css_first("h1.product_title.entry-title, h1.product_title, h1.entry-title")
        if not title_el:
            title_el = tree.css_first("h1")
        data["title"] = title_el.text(strip=True) if title_el else None

        sku_el = tree.css_first(".product_meta .sku, span.sku")
        data["sku"] = sku_el.text(strip=True) if sku_el else None

        ins_el = tree.css_first("p.price ins .woocommerce-Price-amount bdi")
        del_el = tree.css_first("p.price del .woocommerce-Price-amount bdi")
        price_el = tree.css_first("p.price .woocommerce-Price-amount bdi")
        if ins_el and del_el:
            data["price"] = self._parse_price(ins_el.text())
            data["old_price"] = self._parse_price(del_el.text())
        else:
            data["price"] = self._parse_price(price_el.text() if price_el else None)
            data["old_price"] = None

        if data.get("old_price") and data.get("price") and data["old_price"] != data["price"]:
            data["discount_percent"] = round((1 - data["price"] / data["old_price"]) * 100)

        # Brand from product_meta posted_in (categories double as brand for fashion)
        brand_el = tree.css_first(".pwb-single-product-brands a, .product_meta .posted_in a")
        data["brand"] = brand_el.text(strip=True) if brand_el else None

        # Availability from product container class
        cont = tree.css_first("[id^='product-'].product")
        if not cont:
            cont = tree.css_first("body")
        cont_cls = cont.attributes.get("class", "") if cont else ""
        if "outofstock" in cont_cls:
            data["availability"] = "Rupture de stock"
            data["available"] = False
        elif "instock" in cont_cls:
            data["availability"] = "En stock"
            data["available"] = True
        else:
            in_stock = tree.css_first("p.stock.in-stock")
            out_stock = tree.css_first("p.stock.out-of-stock")
            if in_stock:
                data["availability"] = "En stock"
                data["available"] = True
            elif out_stock:
                data["availability"] = "Rupture de stock"
                data["available"] = False
            else:
                data["availability"] = None
                data["available"] = None

        desc_el = tree.css_first("#tab-description, div.woocommerce-product-details__short-description")
        if desc_el:
            data["description"] = re.sub(r"\s+", " ", desc_el.text(strip=True))[:2000]
        else:
            data["description"] = None

        specs = {}
        for row in tree.css("table.shop_attributes tr, table.woocommerce-product-attributes tr"):
            k = row.css_first("th")
            v = row.css_first("td")
            if k and v:
                key = k.text(strip=True)
                val = v.text(strip=True)
                if key and val:
                    specs[key] = val
        data["specifications"] = specs

        images = []
        for a in tree.css(".woocommerce-product-gallery__image a[href]"):
            src = a.attributes.get("href", "")
            if src and not src.startswith("data:") and src not in images:
                images.append(src)
        if not images:
            for img in tree.css(".woocommerce-product-gallery img"):
                src = img.attributes.get("data-large_image") or img.attributes.get("src")
                if src and not src.startswith("data:") and src not in images:
                    images.append(src)
        data["images"] = images[:10]
        return data


def get_scraper(logger: logging.Logger) -> DrestScraper:
    return DrestScraper(logger)
