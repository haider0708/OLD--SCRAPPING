#!/usr/bin/env python3
"""
totaltunisia.com scraper — Shopify, Cloudflare CDN but no bot protection (httpx).
Categories: /collections/{slug}
Pagination: ?page=N
Price format: "123,000 DT" (comma = thousands sep when 3 digits)
"""
import asyncio
import logging
import re
...
BASE = "https://totaltunisia.com"


class TotalTunisiaScraper(FastScraper):
    """HTTP scraper for totaltunisia.com (Shopify)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("totaltunisia", logger)
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        for a in tree.css("a[href*='/collections/']"):
            ...
            # Skip 'all' and paginated URLs
            if path in ("collections/all", "collections") or "?" in path or "/products/" in path:
                continue
            seen.add(href)
            categories.append({
                "name": name,
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        for item in tree.css("div[data-product-item], .productitem, .grid-product, .product-grid-item"):
            product = {}

            link = item.css_first("a[data-product-page-link], a[href*='/products/']")
            if not link:
                continue
            ...
            name_el = item.css_first("h2.productitem--title a, h2.productitem--title, .productitem--title, h2 a, h3 a")
            if name_el:
                product["name"] = self._clean(name_el.text())

            img = item.css_first("img.productitem--image-primary, img[data-src], img")
            if img:
            ...
            price_el = item.css_first("div.price.productitem__price span.money, span.money, .productitem--price span")
            sale_el = item.css_first(".productitem--price-sale span.money, .price--on-sale span.money")
            compare_el = item.css_first(".productitem--price-compare span.money, .price--compare-at span.money, s span.money")

            if sale_el and compare_el:
                product["price"] = self._parse_price(sale_el.text())
                product["old_price"] = self._parse_price(compare_el.text())
            elif price_el:
    ...
    def has_next_page(self, html: str, current_page: int) -> bool:
        ...
        return tree.css_first(
            f"li.pagination--next a, a[rel='next'], a[href*='page={current_page + 1}']"
        ) is not None
    ...
    def extract_pagination_from_html(self, html: str) -> dict:
        ...
        for a in tree.css(".pagination a, nav.pagination a"):
            try:
                ...
                pass
        has_next = tree.css_first("li.pagination--next a, a[rel='next']") is not None
    ...
    def extract_product_details(self, html: str, product: dict) -> dict:
        ...
        name_el = tree.css_first("h1.product-title, h1.productview--title, h1")
        if name_el:
            details["name"] = self._clean(name_el.text())
        price_el = tree.css_first("span.money, .productview--price span")
        if price_el:
            details["price"] = self._parse_price(price_el.text())
        desc_el = tree.css_first(".productview--description, .product-description, [itemprop='description']")
        if desc_el:
            details["description"] = self._clean(desc_el.text())

        # Shopify: try /products/{handle}.js for SKU and barcode
        # In HTML: "UGC" label (their custom barcode field) — scan for it
        details["sku"] = None
        details["barcode"] = None
        sku_el = tree.css_first("[itemprop='sku'], .product__sku, span.sku")
        if sku_el:
            details["sku"] = self._clean(sku_el.text())
        # Scan for UGC (barcode) and SKU in product meta sections
        for el in tree.css("p, span, li, dt, dd, div"):
            txt = el.text(strip=True)
            if re.match(r"^UGC\b", txt, re.IGNORECASE):
                m = re.search(r"UGC\s*[:\-]?\s*(\S+)", txt, re.IGNORECASE)
                if m:
                    details["barcode"] = m.group(1)
            elif re.match(r"^(SKU|UGS|Réf|Ref|Référence)\s*[:\-]", txt, re.IGNORECASE):
                m = re.search(r"[:\-]\s*(\S+)", txt)
                if m and not details["sku"]:
                    details["sku"] = m.group(1)
        images = []
        for img in tree.css(".productview--media img, .product-images img"):
    ...
    async def scrape(self) -> dict:
        self.logger.info("Starting totaltunisia scrape")
...
def get_scraper(logger: logging.Logger) -> TotalTunisiaScraper:
    return TotalTunisiaScraper(logger)
