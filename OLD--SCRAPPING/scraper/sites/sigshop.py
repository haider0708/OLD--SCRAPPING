#!/usr/bin/env python3
"""
SIG Shop (sig-shop.tn) scraper.
Fast: WordPress + WooCommerce (Electro theme), static HTML.
"""
import logging
import re
...
class SigShopScraper(FastScraper):
    """Fast scraper for sig-shop.tn (WooCommerce)."""

    def __init__(self, logger: logging.Logger):
        super().__init__("sigshop", logger)
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        top_blocks = tree.css("ul#menu-menu-1 > li.menu-item")
        ...
        for top_li in top_blocks:
            ...
            if not top_name or not top_url or top_url in seen:
                continue
            if "/categorie-produit/" not in top_url and "/product-category/" not in top_url:
                continue
            seen.add(top_url)

            top_cat = {"name": top_name, "url": top_url, "level": "top", "low_level_categories": []}

            for low_li in top_li.css("ul.sub-menu > li.menu-item"):
    ...
    def extract_products_from_html(self, html: str) -> List[dict]:
        ...
        for item in tree.css("li.product.type-product, ul.products li.product"):
            ...
            name_el = item.css_first("h2.woocommerce-loop-product__title")
...
def get_scraper(logger: logging.Logger) -> SigShopScraper:
    return SigShopScraper(logger)
