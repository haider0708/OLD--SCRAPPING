...
class TunisiaNetScraper(FastScraper):
    ...
    async def download_frontpage(self) -> Path:
        """Use Playwright for frontpage with retry — tunisianet's Cloudflare needs fresh context."""
        ...
        async with async_playwright() as pw:
            ...
            for attempt in range(1, 4):
                ctx = await browser.new_context(user_agent=random_ua(), locale="fr-FR")
                await ctx.add_init_script(STEALTH_JS)
                page = await ctx.new_page()
                try:
                    resp = await page.goto(self.base_url, wait_until="domcontentloaded", timeout=60000)
                    await page.wait_for_timeout(8000)
                    try:
                        await page.wait_for_selector("li.level-1.parent", timeout=10000)
                    except Exception:
                ...
                if status == 200 and not is_cf and len(html) > 50000:
                    break
                self.logger.warning(f"Attempt {attempt}: status={status}, len={len(html)}, cf={is_cf}")
                if attempt < 3:
