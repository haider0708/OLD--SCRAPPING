...
class ExpertGamingScraper(FastScraper):
    ...
    async def download_frontpage(self):
        """Download frontpage. Try httpx first (now bypasses CF without help);
        fall back to Playwright with networkidle + 20s wait if CF blocks us."""
        import httpx
        output_path = self.html_dir / "frontpage.html"

        # Try httpx with a real-Chrome UA — works currently on expert-gaming.tn
        try:
            async with httpx.AsyncClient(
                ...
                if r.status_code == 200 and len(r.text) > 50000:
                    ...
                    self.logger.info(f"✓ Saved (httpx): {output_path} ({len(r.text):,} bytes)")
                    return output_path
                self.logger.warning(f"httpx returned {r.status_code} / {len(r.text)}B — falling back to Playwright")
        except Exception as e:
            self.logger.warning(f"httpx failed ({e}); falling back to Playwright")

        # Playwright fallback — wait for menu OR networkidle (Cloudflare JS challenge needs ~5-10s)
        ...
        try:
            ...
            try:
                await page.wait_for_selector("ul#menu-notre-boutique", state="attached", timeout=20000)
            except Exception:
                self.logger.warning("Menu selector not found after networkidle")
            html = await page.content()
        finally:
            await page.close()

        save_text_atomic(html, output_path, self.logger)
        self.logger.info(f"✓ Saved (Playwright): {output_path} ({len(html):,} bytes)")
        return output_path
    ...
    async def fetch_html_with_meta(self, url: str, raise_on_error: bool = False) -> dict:
        """Try httpx first (currently bypasses CF), fall back to Playwright."""
        import httpx
        ...
        try:
            async with httpx.AsyncClient(
                ...
                r = await c.get(url)
                if r.status_code == 200 and r.text and not is_blocked_response(r.text, r.status_code):
                    return {
                        "html": r.text,
            ...
            self.logger.debug(f"  httpx failed for {url}: {e}, trying Playwright")
