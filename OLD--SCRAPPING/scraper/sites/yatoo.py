#!/usr/bin/env python3
"""
Yatoo.com.tn scraper — PrestaShop. Categories at /{id}-{slug}, pagination ?page=N.
"""
...
class YatooScraper(FastScraper):

    def __init__(self, logger: logging.Logger):
        super().__init__("yatoo", logger)
    ...
    def _absolute_url(self, href: str) -> str:
        ...
        if href.startswith("//"):
            return "https:" + href
        if href.startswith("/"):
            return f"https://yatoo.com.tn{href}"
        return f"https://yatoo.com.tn/{href}"
    ...
    def extract_categories_from_html(self, html: str) -> dict:
        ...
        # Try nav/header menu structures
        top_items = tree.css(
            "nav#top-menu > ul > li, ul.top-menu > li, "
            "ul.menu > li.category, ul.menu > li"
        )
        ...
        for top_li in top_items:
            ...
            # Sub-categories
            for sub_link in top_li.css("ul a, .dropdown-menu a"):
    ...
    def extract_products_from_html(self, html: str) -> List[dict]:
        ...
        for card in tree.css("article.js-product-miniature, div.js-product-miniature, article.product-miniature"):
            ...
            name_el = card.css_first("h2.product-title, h3.product-title, h2.product_name")
            ...
            image = None
            if img:
                image = img.attributes.get("data-src") or img.attributes.get("data-full-size-image-url") or img.attributes.get("src")
    ...
    def extract_pagination_from_html(self, html: str) -> dict:
        ...
        for a in tree.css(".page-list a, ul.pagination a"):
            try:
                ...
                pass
        has_next = tree.css_first("nav.pagination a.next, a[rel='next']") is not None
    ...
    async def scrape_product_details(self, url: str) -> dict:
        ...
        price_el = tree.css_first(
            ".current-price span[itemprop='price'], span.current-price span, "
            "span[itemprop='price'], .product-prices .current-price span"
        )
        if price_el:
            content = price_el.attributes.get("content")
            data["price"] = self._parse_price(content or price_el.text())
        else:
            meta = tree.css_first('meta[property="product:price:amount"]')
            data["price"] = self._parse_price(meta.attributes.get("content")) if meta else None
        ...
        for script in tree.css('script[type="application/ld+json"]'):
            ...
            if brand:
                break
        if not brand:
            brand_el = tree.css_first(".product-manufacturer a, .product-manufacturer span")
            if brand_el:
        ...
        desc_el = tree.css_first("#description, .product-description, div[itemprop='description']")
        if desc_el:
        ...
        images = []
        for img in tree.css(".product-cover img, .thumb-container img, .product-images img, .js-thumbnails img"):
...
def get_scraper(logger: logging.Logger) -> YatooScraper:
    return YatooScraper(logger)
