#!/usr/bin/env python3
"""
zanimax.tn scraper — Shopify (httpx).
...
BASE = "https://www.zanimax.tn"


class ZanimaxScraper(FastScraper):

    def __init__(self, logger: logging.Logger):
        super().__init__("zanimax", logger)
    ...
    def extract_products_from_html(self, html: str, category_info: dict = None) -> List[dict]:
        ...
        seen_urls = set()
        for item in tree.css("li.grid__item, .product-item, .card-wrapper, .pr_grid_item, .product.nt_pr"):
            product = {}
            ...
            name_el = item.css_first(".product-title a, h3 a, h2 a, .card__heading a, .card__title a")
            if name_el:
            ...
            sale = item.css_first(".price__sale .price-item--sale, .price-item--sale, .price-sale, .product-price-sale")
            if sale:
                product["price"] = self._parse_price(sale.text())
                reg = item.css_first(".price__regular .price-item--regular, .price-item--regular, .compare-price")
                if reg:
                    product["old_price"] = self._parse_price(reg.text())
            else:
                price_el = item.css_first(".price__regular, .price, span.price, .product-price")
                if price_el:
                    product["price"] = self._parse_price(price_el.text())

            product["shop"] = "zanimax"
...
def get_scraper(logger: logging.Logger) -> ZanimaxScraper:
    return ZanimaxScraper(logger)
